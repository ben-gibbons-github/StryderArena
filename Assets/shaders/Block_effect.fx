...
float2 TexCo= float2(clamp(input.texCoord.x*TexMult.x-(floor(input.texCoord.x*TexMult.x)),0,1),clamp(input.texCoord.y*TexMult.y-(floor(input.texCoord.y*TexMult.y)),0,1));

//normal map
  	float3 normalFromMap = (tex2D(NormalMapSampler,TexCo));
...
//texture
	float4 Texture=tex2D(DiffuseTextureSampler,TexCo);
