...
float4 ComputeParticlePosition2(float3 position, float3 velocity,
                               float age, float normalizedAge)
{
    float startVelocity = length(velocity);
...
VertexShaderOutput ParticleVertexShader(VertexShaderInput input)
{
	...
	output.ScreenPos=float2(0.5*output.Position.xy/output.Position.w+float2(0.5,0.5));
	output.ScreenPos.y=1-output.ScreenPos.y;
	output.Distance=distance(CameraPos,ComputeParticlePosition2(input.Position, input.Velocity,
                                              age, normalizedAge));
...
{
//float4 base =tex2D(DepthSampler,float2(input.ScreenPos.x*1.5+0.5,1-(input.ScreenPos.y/2+0.5)));
//float4 base= tex2D(DepthSampler,input.ScreenPos);
    return tex2D(Sampler, input.TextureCoordinate) * input.Color;//*clamp(((base.x+base.y+base.z)*900+150-input.Distance)/200,0,1);
	//return base;
}
